    ...
    if task and task['content_message_id']:
        ...
        if is_repost:
            ...
            try:
                ...
                # If caption too long, fallback to forwarding + separate control message
                if 'caption' in error_str and 'long' in error_str:
                    ...
                    try:
                        # Forward the original message (will show "Forwarded from" but works)
                        ...
                        # Send separate control message with buttons
                        control_msg = await context.bot.send_message(
                            chat_id=query.message.chat_id,
                            text=get_text('task_message_current_prompt', context),
                            reply_markup=InlineKeyboardMarkup(keyboard)
